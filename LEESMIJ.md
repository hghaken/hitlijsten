...
| Vaste paginakop | de `h1` en de regel eronder blijven staan bij het scrollen, net onder de menubalk. Het omwikkelen (`.kopvast`) doet het scriptje in `basis.html` en niet de 29 sjablonen zelf; de hoogte van de menubalk komt uit een gemeten `--menuhoogte`, want die balk wrapt en een vast getal zou de kop er bij een smal venster onderdoor laten schuiven. Onder 760 px plakt hij niet, dezelfde afweging als bij de menubalk daar. Ook de **tabelkop** blijft staan bij een `past`-tabel (≤100 rijen, waar de pagina scrolt in plaats van het vak): `sticky` werkt daar tegen de scrollport van `.tabelvak` en die scrolt niet, dus krijgt zo'n vak `overflow: visible` — maar **alleen als de tabel horizontaal past**, want overflow-x en -y zijn niet los in te stellen en een brede matrix zou zijn schuifbaarheid verliezen. Het scriptje meet dat per tabel, opnieuw bij elke maatverandering. De bovenmarge van `main` zit **in** het vak (negatieve marge + gelijke padding), anders springt de titel 1,6 rem omhoog op het moment dat hij vastplakt; en de dekking is gelijk aan die van de menubalk (45%), want die twee raken elkaar en bij een verschil zie je de naad en oogt de onderste band dicht — de blur doet het leeswerk, niet de donkerte. Boven **een derde van de vensterhoogte** aan vaste balken laat het scriptje de paginakop weer meescrollen: op een half venster wrapt de menubalk naar vier regels en de ondertitel naar drie, en dan lees je door een kier |
