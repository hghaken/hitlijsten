...
| Tweetalig (fase 1) | NL/EN-knop rechtsboven naast Aanmelden (cookie `taal`, route `/taal/<code>`); de menubalk, footer, alle lijstpagina's en DJ Export zijn vertaald via `web/vertalingen.py` (NL-tekst als sleutel, Nederlands als vangnet) plus taal-condities voor lange proza-blokken; de specials (weekbericht, records, zoeken, disclaimer, handleiding enz.) zijn bewust nog Nederlands — fase 2 |
