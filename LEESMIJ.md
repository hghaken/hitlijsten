...
| VirtualDJ | upload je `database.xml` of — makkelijker — de **backup-zip** van VirtualDJ (Instellingen → Backup; daar zit de volledige database in, ook die van losse schijven), kies lijst + jaargang + top-N + strengheid (vier niveaus, van exacte sleutel tot fuzzy met twijfel-label) en download een `.vdjfolder`-playlist in klassementvolgorde met de nummers uit je eigen bibliotheek — streaming/netsearch optioneel, lokaal bestand wint altijd, hoogste bitrate bij dubbelen, en wat ontbreekt staat er als boodschappenlijst bij; er wordt niets bewaard |
