...
| Vormgeving | menubalk in twee rijen (de lijsten boven, de extra's en het beheer gedempt eronder), doorschijnend over de banner en met blur zodra er gescrold is. De bovenste rij is sinds 22-08-2026 een **raster** en geen flexrij: met `flex-wrap` bepaalde de nav waar Handleiding/Disclaimer/Aanmelden/taal terechtkwamen, en die sprongen bij elke schermbreedte een andere kant op. Nu heeft die rechterkolom een vaste plek naast het merk; past de nav er niet meer naast (onder 1250px), dan zakt **alleen de nav** naar een tweede regel. De vier links zijn onderling ook een flexrij met `nowrap` — als inline tekst braken ze middenin af — en onder 760px worden ze kleiner zodat ze op een telefoon náást het merk passen; aanmelden is daarbij een **sleutelicoontje** geworden (🔑, afgemeld 🔓) en de taalknop een **vlaggetje** — de vlag van de taal waar je naartoe gaat, als **SVG** en niet als emoji, want Windows tekent emoji-vlaggen als de kale letters NL en GB (dezelfde reden als bij `_vlag.html`); de Union Jack is vereenvoudigd, want op zestien pixels breed zie je de verspringing in de diagonalen toch niet. Beide met de naam in `title` en `aria-label` — dat is beheer en geen bezoekersfunctie, en het scheelt de zeventig pixels die op 320px net het verschil maakten. Breken ze toch een keer, dan lijnt de rij vanaf boven uit zodat het merk op de eerste regel blijft. Nagemeten op 320/375/762/1200/1251/1270/1400px in beide talen — overal één regel; tabellen tot 100 rijen krijgen hun volle hoogte (geen binnenste scrollbalk), daarboven een scrollvak van 78vh; onder de 760px compact en niet-plakkend |
