...
| Vaste paginakop | de `h1` en de regel eronder blijven staan bij het scrollen, net onder de menubalk. Het omwikkelen (`.kopvast`) doet het scriptje in `basis.html` en niet de 29 sjablonen zelf; de hoogte van de menubalk komt uit een gemeten `--menuhoogte`, want die balk wrapt en een vast getal zou de kop er bij een smal venster onderdoor laten schuiven. Onder 760 px plakt hij niet, dezelfde afweging als bij de menubalk daar. **Nog open:** bij een tabel met de klasse `past` (≤100 rijen, de pagina scrolt in plaats van het vak) verdwijnt de *tabelkop* wél uit beeld — `position: sticky` werkt daar tegen de scrollport van `.tabelvak`, en die scrolt niet. Weghalen van `overflow` zou dat oplossen maar kost het horizontaal scrollen op smalle schermen |
